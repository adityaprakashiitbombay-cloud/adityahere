...
export default function ExpertisePage({ playClickSound }) {
  ...
  const ideologyTags = data?.ideologies?.tags || [];
  const heroQuote = data?.ideologies?.quote || data?.hero?.quote || "Inspired by no one.";
  const quotesList = data?.artworksAndQuotes || [];
      ...
      <div className="brutal-card p-6 bg-[#050505] border-3 border-white shadow-[6px_6px_0px_0px_#00E5FF]">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4">
          <div>
            ...
            <h1 className="text-3xl font-black text-white mt-2 tracking-tight flex items-center gap-2">
              <MatrixDecodeText text="Expertise & Worldview" className="text-white" />
              {isAdmin && (
                <button
                  onClick={() => setAdminModalOpen(true)}
                  className="bg-[#39FF14] text-black font-black px-2 py-0.5 text-[10px] border border-white shadow-[2px_2px_0px_0px_#ffffff] cursor-pointer"
                >
                  ✏️ EDIT
                </button>
              )}
      ...
      <AnimatePresence>
        {adminModalOpen && (
          <AdminEditModal
            ...
            initialTab={activeTab === 'quotes' ? 'quotes' : activeTab === 'ideology' ? 'ideologies' : 'expertise'}
          />
        )}
      </AnimatePresence>
    </motion.div>
